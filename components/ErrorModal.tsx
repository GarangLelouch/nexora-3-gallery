  ...
  return (
    <div
      ...
      <div
        ...
        {message.map((m, i) => (
          <p key={i} className="text-gray-400 text-sm mt-1">{m}</p>
        ))}
        <div className="mt-8 flex justify-center gap-4">
          <button
            onClick={onSelectKey}
            className="px-8 py-2 rounded-lg bg-purple-600 hover:bg-purple-700 text-white font-semibold transition-colors focus:outline-none">
            Clé API
          </button>
          <button
            onClick={onClose}
            className="px-8 py-2 rounded-lg bg-gray-600 hover:bg-gray-500 text-white font-semibold transition-colors focus:outline-none">
            Fermer
          </button>
        </div>
      </div>
